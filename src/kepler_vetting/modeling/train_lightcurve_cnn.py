...
def normalize_from_train(
    ...
    if not np.isfinite(mean):
        raise ValueError("local_view train mean is not finite")

    if not np.isfinite(std) or std == 0.0:
        raise ValueError("local_view train std is invalid")
...
def main() -> None:
    ...
    x_raw = data["local_view"].astype(np.float32)
    ...
    if x_raw.ndim != 2:
        raise ValueError(f"local_view must be 2D; got shape {x_raw.shape}")

    if x_raw.shape[0] != y.shape[0]:
        raise ValueError(
            "local_view and labels row counts differ: "
    ...
    for seed in tqdm(EVAL_SEEDS, desc="local CNN seeds"):
        set_seed(seed)
        ...
        model = LocalViewCNN(input_length=x_raw.shape[1]).to(device)
        ...
        for epoch in epoch_iter:
            ...
            val_auc = (
                roc_auc_score(val_targets.astype(np.int64), val_scores)
        ...
        for split_name, indices in splits.items():
            ...
            dummy_prediction_frame = pd.DataFrame(
                {
            ...
            prediction_frames.append(dummy_prediction_frame)
    ...
    print(
        metrics[metrics["model"] == "local_view_cnn"][
            [
                "seed",
                "split",
                "loss",
