...
from kepler_vetting.modeling.train_fused_transit_set_model import (
    TransitSetEncoder,
...
LOCAL_VIEW_KEY = "local_view"

METRICS_DIR = RUN_METRICS_DIR
MODEL_DIR = RUN_MODEL_DIR

PER_SEED_METRICS_PATH = METRICS_DIR / "fused_local_transit_set_model_metrics_by_seed.csv"
SUMMARY_METRICS_PATH = METRICS_DIR / "fused_local_transit_set_model_metrics_summary.csv"
PREDICTIONS_PATH = METRICS_DIR / "fused_local_transit_set_model_predictions.csv"
TRAINING_HISTORY_PATH = METRICS_DIR / "fused_local_transit_set_model_training_history.csv"
MODEL_PATH = MODEL_DIR / "fused_tabular_local_transit_set_cnn.pt"


class FusedTabularLocalTransitSetCNN(nn.Module):
    def __init__(
        self,
        ...
        fused_dim = 64 + self.transit_encoder.output_dim + 16
...
def train_one_epoch_local_transit(
    model: nn.Module,
    loader: DataLoader,
    ...
    for batch_local, batch_transit, batch_mask, batch_tabular, batch_y in loader:
...
@torch.no_grad()
def predict_local_transit_model(
    model: nn.Module,
    loader: DataLoader,
    criterion: nn.Module,
    device: torch.device,
...
def main() -> None:
    ...
    x_local_raw = data[LOCAL_VIEW_KEY].astype(np.float32)
    ...
    if x_local_raw.ndim != 2:
        raise ValueError(f"{LOCAL_VIEW_KEY} must be 2D; got shape {x_local_raw.shape}")

    if x_local_raw.shape[0] != y.shape[0]:
        raise ValueError(
            f"{LOCAL_VIEW_KEY} and labels row counts differ: "
    ...
    print("local_view_key:", LOCAL_VIEW_KEY)
    print("n_rows:", y.shape[0])
    print("local_view_shape:", x_local_raw.shape)
    ...
    for seed in tqdm(EVAL_SEEDS, desc="fused local transit-set model seeds"):
        set_seed(seed)
        ...
        train_loader = make_local_transit_loader(
        ...
        eval_loaders = {
            split_name: make_local_transit_loader(
        ...
        model = FusedTabularLocalTransitSetCNN(
        ...
        for epoch in epoch_iter:
            train_loss = train_one_epoch_local_transit(
                model=model,
                loader=train_loader,
                optimizer=optimizer,
                criterion=criterion,
            ...
            val_loss, val_logits, val_targets = predict_local_transit_model(
                model=model,
        ...
        for split_name, indices in splits.items():
            split_loss, logits, targets = predict_local_transit_model(
                model=model,
        ...
        if seed == FINAL_MODEL_SEED:
            final_model_payload = {
                ...
                "local_view_key": LOCAL_VIEW_KEY,
                "architecture": "FusedTabularLocalTransitSetCNN",
            }
    ...
    print("TRAIN_FUSED_LOCAL_TRANSIT_SET_MODEL_OK")
